...
document.addEventListener("DOMContentLoaded", function (e) {
    var downloadCategoryParent = $('download_category_parent');

    if(downloadCategoryParent) {
        showUnderDomainCategory(downloadCategoryParent.value);

        downloadCategoryParent.on('change', function () {
            showUnderDomainCategory(this.value);
        });
    }
});

function showUnderDomainCategory(val) {
    if(val == 1) {
    	$('download_category_under_domain').style.display = "table-row"
    } else {
        $('download_category_under_domain').style.display = "none"
	}
}
//-->